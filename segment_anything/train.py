...
def main(batch_size,lr,epoch,username,useroptimizer,ModelName):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for val_data in val_bar:
                ...
                # evaluate_metrics(val_labels, predict_y.cpu().numpy())
                # predict_y = torch.max(outputs, dim=1)[1]
                # val_labels.extend(val_labels.cpu().numpy())
                # val_preds.extend(predict_y.cpu().numpy())

                val_bar.desc = "valid epoch[{}/{}]".format(epoch + 1,
                                                           epochs)

        val_accurate = acc / val_num
        train_acc_history.append(val_accurate)
    ...
    # evaluate_metrics(val_labels, val_preds)
    plot_loss(train_loss_history)
    plot_accuracy(train_acc_history)
